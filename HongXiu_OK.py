...
class HongXiuOk:
    ...
    def __init__(self, save_path):
        ...
        self.headers = {'Host': self.Host, 'User-Agent': self.UA}
    ...
    def getHtml(self, url):
        ...
        for TryCount in range(10):
            try:
                ...
                html = requests.get(url, headers=self.headers, timeout=40, allow_redirects=False)
            ...
            try:
                ...
                if book_root_info is not None:
                    ...
                    if book_link_temp:
                        ...
                        book_type = book_root_info.xpath('//p[@class="tag"]/span[1]/text()')
                        book_words_count = book_root_info.xpath('//p[@class="tag"]/span[3]/text()')

                        book_info = list(zip(book_link, book_name, book_author, book_type, book_words_count))
                        book_keys = ('book_link', 'book_name', 'book_author', 'book_type', 'book_words_count')
                ...
                print("获取第{}页 小说链接及标题失败 Error:{}".format(re.findall('pageNum=\d+?', url)[0], e))
                return
    ...
    def get_category(self, gender_code):
        ...
        """
        url = 'https://www.hongxiu.com/free/all?gender={}&catId=-1'.format(gender_code)
        r = self.getHtml(url)
        if r:
            try:
                root = etree.HTML(r)
                category_code = root.xpath('//ul[@type="category"]/li/@data-id')[2:]
                category_name = root.xpath('//ul[@type="category"]/li/a/text()')[2:]
    ...
    def get_catalog_info(self, book_info):
        if book_info:
            ...
            if html:
                try:
                    root = etree.HTML(html)
                    catalog_info_temp = root.xpath('//*[@id="j-catalogWrap"]/div[2]/div/ul')[0]
                    catalog_link = catalog_info_temp.xpath('//li[@data-rid]/a/@href')
                    catalog_link = ['https:' + item for item in catalog_link]
                    catalog_name = catalog_info_temp.xpath('//li[@data-rid]/a/text()')
    ...
    def create_folders(self, gender, type_name, book_name):
        ...
        global book_count
        book_count += 1
        folder_name_list = [self.save_path, gender, type_name, "%05d " % book_count + book_name]
        # 根据字符拼接文件路径
        for item in folder_name_list:
            folder_path = path.join(folder_path, item)
        # 判断文件夹是否存在
        if os.path.exists(folder_path):
            print("文件夹已存在\t{}".format(folder_path))
            return folder_path
        else:
            ...
            try:
                ...
                print("文件夹创建失败：{}".format(folder_path))
    ...
    def save_content(self, save_full_path, tile_content, file_count):
        ...
        if tile_content:
            ...
            if os.path.isfile(save_file_path):
                print("文件已存在\t{}".format(save_file_path))
            else:
                ...
                with open(save_file_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write('\n')
                    f.write(content)
                    print("下载完成\t{}--{}---\t线程共：{}".format(os.path.split(save_full_path)[-1], file_name_d,threading.active_count()))

                # # 合并写入
                # save_file_path = os.path.join(save_full_path, "0 " + os.path.split(save_full_path)[-1].split(" ")[1] + '.txt')
                ...
                #     f.write(content)
                #     f.write('\n'*3)
                #     print("合并写入成功 \t{}".format("0 " + file_name + '.txt'))

    # 合并为一
    def join_txt(self, dir_path):
        for (spath, list_subfolders, list_file_names) in os.walk(dir_path):
            if list_file_names is not None:
                fullpath = (os.path.join(spath, file) for file in list_file_names)
                for item in fullpath:
                    try:
                        join_file_name = r"000 {}\完本 {}.txt".format(spath, os.path.split(spath)[1].split(' ')[1])
                        with open(item, 'r', encoding='utf-8') as f:
                            content = f.read()
                        with open(join_file_name, 'a+', encoding='utf-8') as f:
                            f.write(content + '\n' * 2)
                    except Exception as e:
                        print("合并失败 -- {}".format(e))
                else:
            ...
            if category:
                try:
                    for item in category:
                        ...
                        # 根据 性别+分类 提取总页码
                        page_num_url = url.format(gender_code, category_code, 1)
                        ...
                        if total_page_num:
                            for pagenum in range(1, total_page_num + 1):
                                # 获取每页内容的小说列表及相关信息
                                book_url = url.format(gender_code, category_code, pagenum)
                                book_info_list = self.get_book_info_list(book_url)
                                # 获取列表成功，遍历
                                if book_info_list:
                                    for book_info in book_info_list:
                                        ...
                                        #创建文件夹
                                        folder_path = self.create_folders(gender_name, category_name,  book_name)
                                        category_info = self.get_catalog_info(book_info)
                                        # 多线程下载
                                        if category_info:
                                            for i, category_item in enumerate(category_info):
                                                t = Thread(target=self.save_content, args=(folder_path, category_item, i+1))
                                                t.setDaemon(True)
                                                t.start()
